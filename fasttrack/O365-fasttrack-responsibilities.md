...
- レコード管理。
  - レコード管理へのアクセス許可の適用。
  - ファイル プランおよび保持スケジュールのラベルおよびポリシーへの変換のガイダンス。
  - 保持ラベルとポリシーの作成。
  - 削除ポリシーの設定。
  - アイテムの廃棄に関するレビュー。
- インサイダー リスクの管理。
  - Office 365 監査ログの有効化。
  - インサイダー リスク管理内での設定の構成。
  - 組み込みのプレイブックを使用したインサイダー リスク ポリシーの作成。
  - 通信コンプライアンスのためのアクセス許可の構成。
  - カスタマイズ可能なテンプレートを使用したコミュニケーション コンプライアンス ポリシーの作成。
  - 警告の監視とレビュー。
- 情報ガバナンス。
  - 情報ガバナンスへのアクセス許可の適用。
  - 保持ラベルの作成。
...
## <a name="microsoft-information-protection"></a>Microsoft 情報保護
...
- ポリシーを使用したラベルの発行 (手動および自動)。
- Microsoft Teams のチャットとチャネルのデータ損失防止 (DLP) ポリシーの作成。
