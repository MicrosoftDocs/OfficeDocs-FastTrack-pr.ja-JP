...
#### <a name="germany-microsoft-cloud-deutschland-mcd"></a>ドイツ Microsoft Cloud Deutschland (MCD)

定義済みの移行時間帯に、標準化されて事前にスケジュールされた 1 日 9 時間、週 5 営業日 (9 x 5) 体制で移行が行われます。移行日あたり 1 つの移行バッチがあります。月曜日の正午 12:00 (UTC) から金曜日午後 9:00 (UTC) までの平日には、移行日が 5 日あります。
...
- 各移行イベントの少なくとも 3 日前に、定義された方法で設定したスケジュールと、移行する特定のメールボックスの一覧を提供します。Notes の移行の場合、21 日前にはスケジュールを必ず設定する必要があります。
...
> [!NOTE]
> 移行には、オンボード中に定義されたセキュリティ要件に準拠しているアカウントのみを使用します。 そのようなアカウントを使用しない場合、移行の遅延が発生する可能性があります。 
...
- 各移行イベントの少なくとも 7 日前に、定義された方法で設定したスケジュールと、移行する特定のユーザー データの一覧を提供します。
